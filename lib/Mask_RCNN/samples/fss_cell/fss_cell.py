...
class CocoConfig(Config):
    """Configuration for training on FSS-Cell.
    ...
    NUM_CLASSES = 1 + 14  # FSS-Cell has 14 classes
...
class CocoDataset(utils.Dataset):
    def load_coco(self, dataset_dir, subset, class_ids=None, return_coco=False):
        ...
        subset: What to load (train_val, test)
    ...
    def load_mask(self, image_id):
        ...
        for annotation in annotations:
            class_id = self.map_source_class_id(
                "fss_cell.{}".format(annotation['category_id']))
            if class_id:
